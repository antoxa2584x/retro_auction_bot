...
- [Водяний знак на фото](#-водяний-знак-на-фото)
...
| 🏷️ **Живий хештег статусу** | Шапка позначається `#активний`, поки аукціон відкритий, і змінюється на `#завершений` після закриття. |
...
| 🖼 **Водяний знак на фото** | Накладає ваш PNG на головне фото кожного аукціону, який ви публікуєте. |
...
- **Живий хештег статусу** — шапка допису позначається `#активний`, поки тривають ставки, і автоматично змінюється на `#завершений` після закриття аукціону (мовою бота). Перезапуск завершеного аукціону повертає позначку назад.
...
5. **Завершення аукціону** — планувальник запускає послідовність закриття, змінює хештег у шапці на `#завершений`, оновлює допис із переможцем і сповіщає його приватно.
...
- **⚙️ Налаштування**
  ...
  - *Водяний знак* — завантажте PNG і оберіть його розташування, розмір і непрозорість; він накладається на головне фото аукціонів, опублікованих з адмін-панелі.
...
Накладайте власний логотип на головне фото кожного аукціону, який ви публікуєте з адмін-панелі.

Відкрийте **⚙️ Налаштування → 🖼 Водяний знак**, завантажте PNG і оберіть, як він розміщується на фото:

| Параметр | Що робить |
|----------|-----------|
| **Зображення** | Сам PNG водяного знака. Завантажується один раз, замінити можна будь-коли. |
| **Накладати водяний знак** | Головний перемикач — зберігає зображення, але припиняє його накладання. |
| **Розташування** | Сітка 3×3: будь-який кут, край або центр. |
| **Розмір** | Ширина у відсотках від ширини фото (типово 25%), тож знак масштабується під будь-яке зображення. |
| **Непрозорість** | 1–100% (типово 100%). |
| **Переглянути** | Показує поточні налаштування на зразковому зображенні, щоб перевірити розташування до публікації. |

> ⚠️ **Надсилайте PNG саме файлом, а не фото.** Telegram стискає фото в JPEG, що знищує прозорість і залишить суцільний прямокутник поверх ваших зображень. Скріпка → **Файл**.

**Область дії:** водяний знак накладається лише на **головне фото аукціонів, опублікованих через адмін-майстер**. Додаткові фото галереї та схвалені вами аукціони від користувачів залишаються без змін. Якщо накладання не вдалося, аукціон публікується з оригінальним фото, а не блокується.

PNG зберігається в базі даних, тож він переноситься разом із резервною копією `auction.sqlite3`.
...
| **sharp** | Накладання водяного знака на фото |
...
│  ├─ watermark.js      # Зберігання водяного знака та накладання на фото
